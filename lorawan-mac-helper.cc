    ...
    : m_region(LorawanMacHelper::EU)
{
 ...
 Ptr<Packet> CreateBeaconPacket() 
    {
        // Create the beacon packet
        Ptr<Packet> packet = Create<Packet>();

        // Create and set the LorawanMacHeader
        LorawanMacHeader macHeader;
        macHeader.SetMType(LorawanMacHeader::BEACON);
        packet->AddHeader(macHeader);

        // Create and set the beacon header
        BeaconHeader beaconHeader;
        Time currentTime = Simulator::Now();
        beaconHeader.SetTime(currentTime.GetSeconds());
        packet->AddHeader(beaconHeader);

        // You can add more information to the beacon packet here if needed

        return packet;
    }
 //   void LorawanMacHelper::SendBeacon()
//{
  //  Ptr<Packet> beaconPacket = CreateBeaconPacket();

    // Example: send the beacon packet
 //   Ptr<LoraNetDevice> device = GetDevice();
   // device->Send(beaconPacket);
//}
